...
5. links RDD is created from rawLinks using the same customPartitioner.
...
—build-cont.sbt
|
—src/main/scala/Part-A-Question-2.scala

Then to compile go to the root folder and use command 
sbt package
 to compile the code. After compiling it will create a jar file in target/scala-2.10/ . In this case it is part-a-question-2_2.10-1.0.jar . Then to run the jar use this command
spark-submit --class "PartAQuestion2” --master spark://10.254.0.147:7077 target/scala-2.10/part-a-question-2_2.10-1.0.jar hdfs:///user/ubuntu/cs-838/part-a/workload/web-BerkStan.txt 40 10

where:
1. “PartAQuestion2” is the class name of the class we want to run in Part-A-Question-2.scala . 
2.  spark://10.254.0.147:7077 is the url where spark master is running. 
3. target/scala-2.10/part-a-question-2_2.10-1.0.jar is the location of our jar file
